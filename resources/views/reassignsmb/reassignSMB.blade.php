            ...
            <div class="col-sm-12">
                <div class="card" style="margin-top:2em;">
                    ...
                    <div class="card-body" style="background:#fafafa">
                        <center>
                            <br>
                            <form action="/updaterangesmb" method="post">
                            ...
                            @foreach ($lists as $list)
                            <?php 
                            $who = DB::table('qrmaster')
                            ->distinct()->get(['stockcode','sonum','asgnto'])
                            ->where('stockcode','=', $list->stockcode)
                            ->where('sonum','=', $list->sonum);

                            $who2 = DB::table('qrmaster')
                            ->where('stockcode','=', $list->stockcode)
                            ->where('sonum','=', $list->sonum)
                            ->where('asgnto','=', $list->asgnto)->count();
                            ...
                            $who3 = DB::table('users')
                            ->get(['name','StaffID'])
                            ->where('StaffID','=', $who->asgnto);
                            ?>

                            @foreach ($who3 as $who3)
                            <?php $who4 = DB::table('qrmaster')
                            ->where('stockcode','=', $list->stockcode)
                            ->where('sonum','=', $list->sonum)
                            ->where('asgnto','=', $who->asgnto)
                            ->count();

							$who22 = DB::table('qrmastersmb')
                            ->where('stockcode','=', $list->stockcode)
                            ->where('sonum','=', $list->sonum)
                            ->where('asgnto','=', $who->asgnto)
                            ->count(); ?>
                            ...
                            <a style="font-size:16px"><span
                                    style="background-color:yellow; border-radius:20px">&nbsp;{{$who3->name}} =
                        ...
                        <table>
                            ...
                            <tr>
                                ...
                                <td>
                                    <center>
                                        <form action="/editreassignsmb" method="post">
                                            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                            ...
                                            <input type="text" name="qrcode" value="{{$list->qrcode}}" hidden />
											  <input type="text" name="number" value="{{$list->number}}" hidden />
											  <input type="text" name="sonum" value="{{$list->sonum}}" hidden />
											<input type="text" name="stockcode" value="{{$list->stockcode}}" hidden />
                                    </center>
                                </td>
                                <td style="text-align:center">
                                    <input class="btn btn-info" style="width:120px" type='submit' value="Save11" />
                                    </form><br>
                                </td>
                            </tr>
                            @endforeach
