...
body {
    ...
    <center>
        <a style="font-size:32px; text-align:center; color: white;font-weight:bold;">WORK ORDER BYPASS REQUEST LIST</a>
    </center>
    ...
    <div class="table-wrapper container-fluid">
        ...
        <table id="tableList" class ="table striped" style="width:100%;">
		<thead>
			<tr>
				<th>#</th>
                <th>SO & STOCK</th>
				...
				<th>ZONE</th>
				<th>MACHINE</th>
                <th>ACTUAL WEIGHT (Smallbag) gram</th>
                <th>UNIT</th>
                <th>WEIGHT TOLERANCE</th>
                <th>CURRENT SMALL BAG<brSEQ</th>
                <th>CURRENT STD BAG<brSEQ</th>
                <th>TYPE</th>
                <th>BYPASSED BY</th>
                <th>STATUS</th>
                <th>ACTION</th>
        ...
        <tr>
         <td>{{$key+1}}</td>   
         <td style="width:13%;">{{$bypass->SO_NO}}<br>{{$bypass->STK_CODE}}<br><b style="color:red;">{{ $bypass->PRD_ID }}</b></td>
         <?php $op = User::where('staffId','=',$bypass->OPER_STAFF_ID)->first(); ?>
         <td style="width:12%;color:black;"><font color="blue">{{$op->name  ?? 'N/A' }}</font><br>
         {{$bypass->Createdate ? Carbon::parse($bypass->Createdate)->formatLocalized('%d %b %Y %I:%M %p') : 'N/A'}}
        </td>
         <td style="width:8%;">{{$bypass->ZONE_NAME}}</td>
         <td style="width:12%;">{{$bypass->machineID}} :<br>{{$bypass->machineName}}</td>
     <td>
    {{ rtrim(rtrim(number_format($bypass->WEIGHT * 1000, 5, '.', ''), '0'), '.') }}
</td>

         <td>gram</td>
<td style="width:30%;">
    @if($bypass->EXCEPTION_TYPE == 'WEIGHT TOLERANCE')
        Stock Weight (KG Per Nos): <br>
        <b style="color:red;">{{ rtrim(rtrim(number_format($bypass->STK_WEIGHT, 5, '.', ''), '0'), '.') }}</b><br>
        
        Current Actual Weight (gram per Smallbag) <b style="color:green;">[a]</b>: <br>
        <b style="color:red;">{{ rtrim(rtrim(number_format($bypass->WEIGHT * 1000, 5, '.', ''), '0'), '.') }}</b><br>
        
        Expected Weight (gram per Smallbag) <b style="color:purple;">[b]</b>: <br>
        <b style="color:red;">{{ rtrim(rtrim(number_format($bypass->STD_WEIGHT, 5, '.', ''), '0'), '.') }}</b><br>
        
        Tolerance (gram) <b>(<b style="color:green;">[a]</b> - <b style="color:purple;">[b]</b>)</b>: <br>
        <b style="color:red;">{{ rtrim(rtrim(number_format($bypass->TOLERANCE_WEIGHT, 5, '.', ''), '0'), '.') }}</b><br>
        
        Tolerable Diff (-) (gram): <br>
        <b style="color:red;">{{ rtrim(rtrim(number_format($bypass->TD_DIFF_MINUS, 5, '.', ''), '0'), '.') }}</b><br>
        
        Tolerable Diff (+) (gram): <br>
        <b style="color:red;">{{ rtrim(rtrim(number_format($bypass->TD_DIFF_PLUS, 5, '.', ''), '0'), '.') }}</b>
    @else
        -
    @endif
</td>
         ...
         <td >{{$bypass->EXCEPTION_TYPE == 'WEIGHT TOLERANCE' ? 'SMALL BAG WEIGHT TOLERANCE' : $bypass->EXCEPTION_TYPE}}</td>
        ...
        <font color="#1B2F5D">
         ...
         Remarks :  {{$bypass->REMARKS}}<br>
		 New Stock Weight Per Nos (KG):  {{$bypass->STK_WEIGHT_NEW}}<br>
         @else
         N/A
        @endif
        </td>
         <td>@if($bypass->STATUS =='P') <h5><span class="badge badge-warning">PENDING</span></h5>
			 @elseif($bypass->STATUS =='R') <h5><span class="badge badge-danger">REJECTED</span></h5> 
			 @else <h5><span class="badge badge-success">COMPLETED</span></h5>
			 @endif</td>
         <td>
         @if($bypass->STATUS =='P') 
...
->table('STK_MST')
->where('STK_CODE',$bypass->STK_CODE)
->first();


?>
<a href="#" style="float:right;" class="approveByPass2" data-id="{{$bypass->EXCEPTION_ID}}" data-sono="{{$bypass->SO_NO}}" data-order="{{$key+1}}" data-type="{{$bypass->EXCEPTION_TYPE}}" data-prid="{{$bypass->PRD_ID}}" data-stock="{{$bypass->STK_CODE}}" data-weight="{{ $getstockcode->STK_WEIGHT }}">
...
@else
         <a href="#" style="float:right;" class="approveByPass2" data-id="{{$bypass->EXCEPTION_ID}}" data-sono="{{$bypass->SO_NO}}" data-order="{{$key+1}}" data-type="{{$bypass->EXCEPTION_TYPE}}" data-prid="{{$bypass->PRD_ID}}" data-stock="{{$bypass->STK_CODE}}" data-weight="0">
         <i class="material-icons approved" style="color: green;" data-toggle="tooltip" data-placement="top" title="Click here to approve !">check_circle</i>
        </a>
        @endif
        @else
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                                ...
                                <input type="hidden" id ="STK_CODE" name="STK_CODE" value="" />
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="row weight" style="display: none;">
                                    <div class="col"><br>
                                    <b><label>Stock Weight (KG) Per Nos</label></b>
                                    <input type="number" step=".00001" class="form-control" style="text-align:left;width:100%;" id="STK_WEIGHT" name="STK_WEIGHT" max="99" required/>
...
$(document).ready(function() {  
           ...
           //$('.approveByPass2').on('click', function() {  
                ...
                if(dataType == 'WEIGHT TOLERANCE'){
                    $('.weight').show();
                    $('#STK_WEIGHT').attr('required',true).val(weight);
                }
                else{
                    $('.weight').hide();
                    $('#STK_WEIGHT').attr('required',false).val(0);
                }
