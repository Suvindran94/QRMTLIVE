...
class AjaxController_live extends Controller
{
...
$getdevice = DB::table("device")->where('deviceId',$deviceId)->first();
  ...
  ->join('moresolist', function($join)
         ...
         $res = strtolower(substr($qrcode,0,3));
          ...
          if($res == "emp"){
            ...
            if($qr === NULL && $qr4 === NULL){
                ...
                $da = array("deviceId"=>$request->deviceId);
                DB::table('userdevice')->where('StaffID','=', $qr2->StaffID )->update($da);
            ...
            return response()->json([
              'type' => 'ERROR',
              'message' => 'Please Register Your QR Code!'
           ]);
        }
          }else{
        ...
        $prints11 = DB::table('qrmaster')
            ...
            ->count();
			  ...
			  if($prints10 == ''){
				    ...
				    return response()->json([
                  'type' => 'ERROR',
                  'message' => 'QRMASTER DATA IS NULL <br>. ' .$prints2->sonum. ' | ' .$prints2->stockcode.'&nbsp'.$qrcode.''
               ]);
         ...
         if(!empty($null5)){
          if($null->dt_printseal !== null){
              if($null->dt_opscancomplete === null){
                 ...
                 DB::table('temp_check_comp')->where('QRCODE',$qrcode)->delete();
                 ...
                 DB::select('CALL SP_CHECK_COMPONENT(?,?,?,?,?)',array($qrcode, $stockcode,$division,$getsonum,$getwarehouse));

                 $getdata = DB::table('temp_check_comp')->where('QRCODE',$qrcode)->get();
      ...
      /*DB::select('CALL SP_INSERT_SFU(?)',array($qrcode));
		     \Log::debug($qrcode.''.' 235'); */
                 ...
                 return response()->json([
                  'type' => 'SUCCESS',
                  'message' => 'Successfully Updated!<br> No. ' .$qrmasterinfo->seq. ' - ' .$qrmasterinfo->particular.'&nbsp'.$qrmasterinfo->particular2.''
               ]);
                ...
                return response()->json([
                  'type' => 'ERROR',
                  'message' => 'Already Scan<br> No. ' .$qrmasterinfo->seq. ' - ' .$qrmasterinfo->particular.'&nbsp'.$qrmasterinfo->particular2.''
               ]);
            ...
            return response()->json([
              'type' => 'ERROR',
              'message' => 'Please Print Sticker First!'
           ]);
    ...
    $checksmb = DB::table('qrmastersmb')->where('qrcode',$qrcode)->count(); //standard packing 
            ...
            if($checksmb > 0){
              ...
              return response()->json([
                'type' => 'ERROR',
                'message' => 'Please Scan Standard Bag Sticker!'
             ]);
       ...
       $null5 = DB::table('users')->where('StaffID',$null->asgnto)->first(); // user
           ...
           if(!empty($null5)){ 
            if($null->dt_printseal !== null){
				...
				try {
						 \Log::debug($qrcode.''.'369');
				DB::select('CALL SP_INSERT_SFU(?)',array($qrcode));
			} catch (\Exception $e) {	
				\Log::emergency($e->getMessage()); 
				try {
						 \Log::debug($qrcode.''.'373');
					DB::select('CALL SP_INSERT_SFU(?)',array($qrcode));
				}
				catch (\Exception $e) {
					\Log::emergency($e->getMessage()); 
					\Log::debug($qrcode.''.' 356');
				}			
			}
      /*DB::select('CALL SP_INSERT_SFU(?)',array($qrcode));
          ...
          return response()->json([
            'type' => 'SUCCESS',
            'message' => 'Successfully Scanned!<br> Small Bag No. ' .$qrmastersmbinfo->number. ' <br> ' .$qrmastersmbinfo->particular.'&nbsp'.$qrmastersmbinfo->particular2.''
         ]);
            ...
            if ($check === NULL){
                ...
                return response()->json([
                  'type' => 'ERROR',
                  'message' => 'Please Scan Small Bag2!'
               ]);
                ...
                return response()->json([
                  'type' => 'ERROR',
                  'message' => 'Already Scanned!<br> Small Bag No. ' .$qrmastersmbinfo->number. ' <br> ' .$qrmastersmbinfo->particular.'&nbsp'.$qrmastersmbinfo->particular2.''
               ]);
            }
          }
      }
      ...
      $var10 = $prints10->pbag;
        ...
        $var10 = $prints4->pbag;
           ...
           $null5 = DB::table('users')->where('StaffID',$null->asgnto)->first();
			...
			try {
					 \Log::debug($qrcode.''.'585');
				DB::select('CALL SP_INSERT_SFU(?)',array($qrcode));
			} catch (\Exception $e) {
					\Log::emergency($e->getMessage()); 
				try {
						 \Log::debug($qrcode.''.'589');
					DB::select('CALL SP_INSERT_SFU(?)',array($qrcode));
				}
				catch (\Exception $e) {
					\Log::emergency($e->getMessage()); 
					\Log::debug($qrcode.''.' 559');
				}			
			}
           ...
           $checkPallet = DB::table('moresolist')->where('sonum',$null->sonum)->where('stockcode',$null->stockcode)->first();
                 ...
                 DB::select('CALL SP_CHECK_COMPONENT(?,?,?,?,?)',array($qrcode, $stockcode,$division,$getsonum,$getwarehouse));
                       ...
                       //Session::flash('message','Successfully Updated!<br> No. ' .$qrmasterinfo->seq. ' - ' .$qrmasterinfo->particular.'&nbsp'.$qrmasterinfo->particular2.'');
						  ...
						  try {
							  	 \Log::debug($qrcode.''.'711');
				DB::select('CALL SP_INSERT_SFU(?)',array($qrcode));
			} catch (\Exception $e) {		
							  	\Log::emergency($e->getMessage()); 
				try {
						 \Log::debug($qrcode.''.'714');
					DB::select('CALL SP_INSERT_SFU(?)',array($qrcode));
				}
				catch (\Exception $e) {
					\Log::emergency($e->getMessage()); 
					\Log::debug($qrcode.''.' 672');
				}			
			}
                ...
                if( DB::table('qrmastersmb')->where('dt_opscancomplete','=', NULL) ->where('qrcode', $qrcode)->update($data2)){
                 ...
                 return response()->json([
                  ...
                  if ($check === NULL){
                      ...
                      DB::select('CALL SP_CHECK_COMPONENT(?,?,?)',array('QR60/100|7865-SO21/00790-17MTA-032G--7AK0K0W0BJ', '17MTA-032G--7AK0K0W0BJ', 'PM'));
                      ...
                      */
                      ...
                      return response()->json([
                        'type' => 'ERROR',
                        'message' => "Please Scan Small Bag1! <br> $var10.''.$total"
                     ]);
